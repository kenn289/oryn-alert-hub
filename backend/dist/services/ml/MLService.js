...
class MLService {
    ...
    async initialize() {
        try {
            logger_1.logger.info('🤖 Initializing ML Service...');
            await tf.ready();
            logger_1.logger.info('✅ TensorFlow initialized');
            ...
            logger_1.logger.error('❌ Failed to initialize ML Service:', error);
            throw error;
        }
    }
    async loadModels() {
        const modelConfigs = [
            {
                ...
                version: '1.0.0',
                path: `${config_1.config.ML.MODEL_PATH}/price-prediction-v1.json`,
                type: 'regression'
            },
            {
                name: 'sentiment-analysis-v1',
                version: '1.0.0',
                path: `${config_1.config.ML.MODEL_PATH}/sentiment-analysis-v1.json`,
                type: 'classification'
            },
            {
                name: 'technical-indicators-v1',
                version: '1.0.0',
                path: `${config_1.config.ML.MODEL_PATH}/technical-indicators-v1.json`,
                type: 'regression'
            }
        ];
        for (const config of modelConfigs) {
            try {
                ...
                logger_1.logger.info(`✅ Loaded model: ${config.name} v${config.version}`);
            }
            catch (error) {
                logger_1.logger.warn(`⚠️ Failed to load model ${config.name}:`, error);
    ...
    async runMLPrediction(data, indicators) {
    ...
    startBackgroundTraining() {
        setInterval(async () => {
            try {
                ...
                logger_1.logger.error('Background training failed:', error);
            }
        }, 24 * 60 * 60 * 1000);
    }
    async retrainModels() {
        logger_1.logger.info('🔄 Starting background model retraining...');
    ...
    async close() {
        logger_1.logger.info('🤖 Closing ML Service...');
        this.isInitialized = false;
        logger_1.logger.info('✅ ML Service closed');
    }
}
